...
std::vector<std::shared_ptr<NWeb::AudioDeviceDescAdapter>> ArkAudioDeviceDescAdapterVectorStructToClass(
    const ArkAudioDeviceDescAdapterVector& struct_value)
{
    ...
    if (struct_value.size > 0) {
        for (int count = 0; count < struct_value.size; count++) {
            std::shared_ptr<NWeb::AudioDeviceDescAdapter> adapter = std::make_shared<ArkAudioDeviceDescAdapterWrapper>(
                ArkAudioDeviceDescAdapterCToCpp::Invert(struct_value.value[count]));
