...
static const std::string REPLY_ERROR_INDEX = "errorIndexList";
static const std::string REPLAY_SUCCESS_COUNT = "successCount";
std::atomic<bool> MigrationManagerAdapterImpl::isConnectSystemUI_(false);

void MigrationManagerAdapterImpl::MigrationListenerAdapterImpl::OnMigrationReply(
    int32_t errorCode, int32_t succussCount, std::vector<int32_t>& errorIndex)
{
    if (!listener_) {
        WVLOG_W("callback_ is not initialize");
    }
    WVLOG_I("on migration replay will be invoked");
    listener_->OnMigrationReply(errorCode, succussCount, errorIndex);
}
    ...
    const AppExecFwk::ElementName& element, const sptr<IRemoteObject>& remoteObject, int resultCode)
{
    ...
    std::string jsonData = Str16ToStr8(reply.ReadString16());
    WVLOG_I("send migration request reply res: %{public}s", jsonData.c_str());
    ...
    if (jsonObj.contains(REPLY_ERROR_INDEX) && jsonObj[REPLY_ERROR_INDEX].is_array()) {
        errorIndexList = jsonObj[REPLY_ERROR_INDEX].get<std::vector<int>>();
    }
    if (jsonObj.contains(REPLAY_SUCCESS_COUNT) && jsonObj[REPLAY_SUCCESS_COUNT].is_number_integer()) {
        successCount = jsonObj[REPLAY_SUCCESS_COUNT];
    }
    WVLOG_I("send migration request successCount: %{public}d", successCount);
    OnMigrationReply(errorCode, successCount, errorIndexList);
