...
template <class ClassName, class BaseName, class StructName>
ArkWebCppToCRefCounted<ClassName, BaseName,
                       StructName>::ArkWebCppToCRefCounted() {
...
ArkWebCppToCRefCounted<ClassName, BaseName, StructName>::Get(StructName *s) {
...
StructName *ArkWebCppToCRefCounted<ClassName, BaseName, StructName>::Invert(
  ...
  ARK_WEB_CPPTOC_DV_LOG("bridge type is %{public}d,this is %{public}ld,capi "
                        ...
                        kBridgeType, (long)bridge, (long)capi_struct,
                        (long)c.get());
...
template <class ClassName, class BaseName, class StructName>
StructName *
ArkWebCppToCRefCounted<ClassName, BaseName, StructName>::GetStruct() {
...
ArkWebCppToCRefCounted<ClassName, BaseName, StructName>::GetBridgeStruct(
  ...
  BridgeStruct *bridgeStruct = reinterpret_cast<BridgeStruct *>(
      ...
      (sizeof(BridgeStruct) - sizeof(StructName)));
  if (bridgeStruct->type_ != kBridgeType) {
    ARK_WEB_CPPTOC_INFO_LOG("bridge type %{public}d - %{public}d is invalid",
...
void ArkWebCppToCRefCounted<ClassName, BaseName, StructName>::DecreRef() const {
  ...
  if (ref_count_.DecreRef()) {
    ARK_WEB_CPPTOC_DV_LOG(
        "delete cpptoc,bridge type is %{public}d,this is %{public}ld",
...
ArkWebCppToCRefCounted<ClassName, BaseName, StructName>::StructIncreRef(
    ark_web_base_ref_counted_t *base) {
  ARK_WEB_CPPTOC_REF_LOG("bridge type is %{public}d", kBridgeType);

  BridgeStruct *bridgeStruct =
      GetBridgeStruct(reinterpret_cast<StructName *>(base));
  if (!bridgeStruct) {
    ARK_WEB_CPPTOC_INFO_LOG(
        "failed to get bridge struct,bridge type is %{public}d", kBridgeType);
    return;
...
template <class ClassName, class BaseName, class StructName>
void ARK_WEB_CALLBACK
ArkWebCppToCRefCounted<ClassName, BaseName, StructName>::StructDecreRef(
    ark_web_base_ref_counted_t *base) {
  ARK_WEB_CPPTOC_REF_LOG("bridge type is %{public}d", kBridgeType);
