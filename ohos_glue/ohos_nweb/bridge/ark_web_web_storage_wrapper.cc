...
ArkWebWebStorageWrapper::ArkWebWebStorageWrapper(
...
ArkWebWebStorageWrapper::GetOrigins() {
  return ArkWebWebStorageOriginVectorStructToClass(
      ark_web_web_storage_->GetOrigins());
}

void ArkWebWebStorageWrapper::GetOrigins(
  ...
  ark_web_web_storage_->GetOrigins(
      new ArkWebWebStorageOriginVectorValueCallbackImpl(callback));
}
...
void ArkWebWebStorageWrapper::GetOriginQuota(
  ...
  if (CHECK_SHARED_PTR_IS_NULL(callback)) {
    ...
    ark_web_web_storage_->GetOriginQuota(
        stOrigin, new ArkWebLongValueCallbackImpl(callback));
  }
...
void ArkWebWebStorageWrapper::GetOriginUsage(
  ...
  if (CHECK_SHARED_PTR_IS_NULL(callback)) {
    ...
    ark_web_web_storage_->GetOriginUsage(
        stOrigin, new ArkWebLongValueCallbackImpl(callback));
  }
