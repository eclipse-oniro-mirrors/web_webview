...
void ArkWebDownloadCallbackImpl::OnDownloadStart(
    const ArkWebString &url, const ArkWebString &user_agent,
    const ArkWebString &content_disposition, const ArkWebString &mime_type,
    long content_length) {
  nweb_download_callback_->OnDownloadStart(
      ArkWebStringStructToClass(url), ArkWebStringStructToClass(user_agent),
      ArkWebStringStructToClass(content_disposition),
      ArkWebStringStructToClass(mime_type), content_length);
}
