...
ArkWebCreateNativeMediaPlayerCallbackImpl::OnCreate(
  ...
  std::shared_ptr<OHOS::NWeb::NWebNativeMediaPlayerHandler>
      nweb_native_media_player_handler = nullptr;
  if (!CHECK_REF_PTR_IS_NULL(handler)) {
    nweb_native_media_player_handler =
        std::make_shared<ArkWebNativeMediaPlayerHandlerWrapper>(handler);
  }

  std::shared_ptr<OHOS::NWeb::NWebNativeMediaPlayerBridge>
      nweb_native_media_player_bridge =
          nweb_create_native_vide_player_callback_->OnCreate(
              nweb_native_media_player_handler, nweb_media_info);
