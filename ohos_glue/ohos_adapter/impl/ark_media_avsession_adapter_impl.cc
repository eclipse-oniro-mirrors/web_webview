...
#include "wrapper/ark_media_avsession_callback_adapter_wrapper.h"
#include "wrapper/ark_media_avsession_metadata_adapter_wrapper.h"
#include "wrapper/ark_media_avsession_position_adapter_wrapper.h"

namespace OHOS::ArkWeb {

ArkMediaAVSessionAdapterImpl::ArkMediaAVSessionAdapterImpl(
    std::shared_ptr<OHOS::NWeb::MediaAVSessionAdapter> ref) : real_(ref) {}
...
bool ArkMediaAVSessionAdapterImpl::RegistCallback(
    ArkWebRefPtr<ArkMediaAVSessionCallbackAdapter> callbackAdapter) {
    if (!(CHECK_REF_PTR_IS_NULL(callbackAdapter))) {
        return real_->RegistCallback(
            std::make_shared<ArkMediaAVSessionCallbackAdapterWrapper>(callbackAdapter));
    }
    return false;
}
...
void ArkMediaAVSessionAdapterImpl::SetPlaybackPosition(
    ...
    if (CHECK_REF_PTR_IS_NULL(position)) {
        ...
        real_->SetPlaybackPosition(
            std::make_shared<ArkMediaAVSessionPositionAdapterWrapper>(position));
    }
}
} // namespace OHOS::ArkWeb
