...
#include "ctocpp/ark_imftext_listener_adapter_ctocpp.h"
    ...
    bool isShowKeyboard, const ArkIMFAdapterTextConfig* config)
{
    ...
    ARK_WEB_CPPTOC_CHECK_PARAM(config, false);

    // Execute
    return ArkIMFAdapterCppToC::Get(self)->Attach(
        ArkIMFTextListenerAdapterCToCpp::Invert(listener), isShowKeyboard, *config);
}
...
void ARK_WEB_CALLBACK ark_imfadapter_on_cursor_update(
    struct _ark_imfadapter_t* self, ArkIMFAdapterCursorInfo cursorInfo)
{
    ARK_WEB_CPPTOC_DV_LOG("capi struct is %{public}ld", (long)self);
    ...
    // Execute
    ArkIMFAdapterCppToC::Get(self)->OnCursorUpdate(cursorInfo);
}
