...
int32_t ARK_WEB_CALLBACK ark_audio_renderer_adapter_create(struct _ark_audio_renderer_adapter_t* self,
    const ArkAudioAdapterRendererOptions* rendererOptions, ArkWebString* str)
{
    ...
    ARK_WEB_CPPTOC_CHECK_PARAM(rendererOptions, 0);

    ARK_WEB_CPPTOC_CHECK_PARAM(str, 0);

    // Execute
    return ArkAudioRendererAdapterCppToC::Get(self)->Create(*rendererOptions, *str);
}
