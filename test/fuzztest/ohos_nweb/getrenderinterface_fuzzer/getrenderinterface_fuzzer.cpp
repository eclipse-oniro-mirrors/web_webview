...
#include "getrenderinterface_fuzzer.h"
...
bool DoSomethingInterestingWithMyAPI(const uint8_t* data, size_t size)
{
    ...
    auto adapter = NWebWindowAdapter::Instance();
    NWebCreateInfo info = adapter.GetCreateInfo(window.GetRefPtr(), GetInitArgs());
    adapter.GetRenderInterface(window.GetRefPtr(), info);
    return true;
}
}
