...
#include "camerasurfacebufferadapter_fuzzer.h"
...
bool CameraSurfaceBufferAdapterFuzzTest(const uint8_t* data, size_t size)
{
    ...
    auto listenerSurfaceAdapter = std::make_shared<CameraSurfaceAdapterImpl>(surface);
    listenerSurfaceAdapter->ReleaseBuffer(nullptr, -1);
    NWebSurfaceAdapter::Instance();
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::CameraSurfaceBufferAdapterFuzzTest(data, size);
    return 0;
}
