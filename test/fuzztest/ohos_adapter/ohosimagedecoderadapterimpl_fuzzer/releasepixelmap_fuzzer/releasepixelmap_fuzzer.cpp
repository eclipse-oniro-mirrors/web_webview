...
#include "releasepixelmap_fuzzer.h"
...
void ReleasePixelMapTest(const std::shared_ptr<OhosImageDecoderAdapter>& adapter, const uint8_t* data, size_t size)
{
...
bool ApplyOhosImageDecoderAdapterFuzzTest(const uint8_t* data, size_t size)
{
    ...
    for (size_t i = 0; i < callCount; ++i) {
        ...
        ReleasePixelMapTest(adapter, data, size);
    }
    return true;
}
